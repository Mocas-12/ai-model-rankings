...
<p align="center"><a href="./README.md">English</a> | <b>简体中文</b> | <a href="./README.ja-JP.md">日本語</a></p>
...
- **🧰 工具箱** — 模型对比（`?vs=` 可分享）、白嫖榜（免费变体）、国产对决、**每日降价榜**（CI 快照当历史库）、月成本计算器。
- **🔍 模型详情** — 点击任意榜单里的模型弹出全维度数据卡，一键送入对比。
- **🖨 水墨拓印** — 一键把「今日五强」或单个模型拓印成水墨分享卡（PNG 下载）。
...
flowchart LR
    ...
    F[每日 CI] -->|成本快照| G[(git: data/snapshots)]
    G -->|diff 最近两份| H[降价榜]
```
...
4. 每日 CI 把 `costPerRequest` 快照提交进 `data/snapshots/`——降价榜 diff 最近两份，纯静态站零后端就有了历史。
5. `app.py` 将页面作为全宽 Streamlit 组件内嵌；页面自行把 iframe 撑到真实内容高度。
...
- **工具箱（拾贰，默认折叠）** — 模型对比（`?vs=` 可分享）、白嫖榜、国产对决、降价榜、算笔账。
- **点击任意模型** — 弹出全维度详情卡；「加入对比」送进对比器；「拓印」生成水墨 PNG。
...
├── js/app.js               # 数据层 + 图表 + 卡片（暴露 window.MB）
├── js/tools.js             # 工具箱：对比 / 白嫖 / 对决 / 降价 / 算账
├── js/card.js              # 模型详情弹层 + 水墨拓印卡
├── scripts/snapshot.py     # 每日成本快照（CI 定时）
├── data/snapshots/         # git 当历史库：降价榜的数据源
...
- `CATS` — 分类榜展示的分类。
- `js/tools.js` 中 `CN_AUTHORS` — 国产对决里计入「国产」的厂商 slug 集合。
